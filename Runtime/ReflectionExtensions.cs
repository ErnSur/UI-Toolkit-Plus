...
namespace QuickEye.UIToolkit
{
    ...
    {
        internal static MethodInfo FindVisualElementOfType { get; } = GetFindVisualElementOfTypeMethod();
        
        private const BindingFlags UQAttributeTargetFlags = BindingFlags.FlattenHierarchy
        ...
        public static (MemberInfo, QAttribute)[] GetQAttributeMembers(this Type type)
        {
            ...
            {
                foreach (var field in type.GetFields(UQAttributeTargetFlags))
                    yield return field;
                foreach (var prop in type.GetProperties(UQAttributeTargetFlags))
                    yield return prop;
            }
        }
